...
function GetSubNet([string]$ResourceGroupName, [string]$VNetName, [int]$Index) {
  if ( !$ResourceGroupName) { throw "ResourceGroupName Required" }
  if ( !$VNetName) { throw "VNetName Required" }

  return (Get-AzureRmVirtualNetwork -ResourceGroupName $ResourceGroupName -Name $VNetName).Subnets[$Index].Name
}
